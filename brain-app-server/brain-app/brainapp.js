...
function setSelectEdgeKeyBackgroundColor(color) {
...
function setEdgeDirectionGradient() {
    ...
    if (apps[0])
        apps[0].setEdgeDirectionGradient();
    if (apps[1])
        apps[1].setEdgeDirectionGradient();
    if (apps[2])
        apps[2].setEdgeDirectionGradient();
    if (apps[3])
        apps[3].setEdgeDirectionGradient();
}
function setEdgeColorByWeight() {
    ...
    if (apps[0])
        apps[0].setEdgeColorByWeight(config);
    if (apps[1])
        apps[1].setEdgeColorByWeight(config);
    if (apps[2])
        apps[2].setEdgeColorByWeight(config);
    if (apps[3])
        apps[3].setEdgeColorByWeight(config);
}
function setEdgeColorByNode() {
    // save edge color setting
    saveObj.edgeSettings.colorBy = "node";
    if (apps[0])
        apps[0].setEdgeColorByNode();
    if (apps[1])
        apps[1].setEdgeColorByNode();
    if (apps[2])
        apps[2].setEdgeColorByNode();
    if (apps[3])
        apps[3].setEdgeColorByNode();
}
function setEdgeNoColor() {
    // save edge color setting 
    saveObj.edgeSettings.colorBy = "none";
    if (apps[0])
        apps[0].setEdgeNoColor();
    if (apps[1])
        apps[1].setEdgeNoColor();
    if (apps[2])
        apps[2].setEdgeNoColor();
    if (apps[3])
        apps[3].setEdgeNoColor();
}
...
function getSource(id, styles) {
    var svgInfo = {}, svg = document.getElementById("svgGraph" + id);
...
function setBrainMode(brainMode, view) {
    switch (view) {
        case tl_view:
...
function brainIconDraggableEvent(view, model, brainSurfaceMode) {
    resetBrain3D();
    var file = 'none';
    if (model === 'ch2') {
        ...
        file = $('#input-select-model').get(0).files[0].name;
    }
    ...
    if (!dataSet) {
        loadExampleData(view, function (view) {
            ...
            switch (view) {
                case tl_view:
                    if (apps[0])
                        apps[0].setDataSet(clonedDataSet);
                    appID = 0;
                    break;
                case tr_view:
                    if (apps[1])
                        apps[1].setDataSet(clonedDataSet);
                    appID = 1;
                    break;
                case bl_view:
                    if (apps[2])
                        apps[2].setDataSet(clonedDataSet);
                    appID = 2;
                    break;
                case br_view:
                    if (apps[3])
                        apps[3].setDataSet(clonedDataSet);
                    appID = 3;
                    break;
            }
            if (appID != -1) {
...
$('#checkbox-thickness-by-weight').on('change', function () {
    if ($('#checkbox-thickness-by-weight').is(":checked")) {
        if (apps[0])
            apps[0].setEdgeThicknessByWeight(true);
        if (apps[1])
            apps[1].setEdgeThicknessByWeight(true);
        if (apps[2])
            apps[2].setEdgeThicknessByWeight(true);
        if (apps[3])
            apps[3].setEdgeThicknessByWeight(true);
    }
    else {
        if (apps[0])
            apps[0].setEdgeThicknessByWeight(false);
        if (apps[1])
            apps[1].setEdgeThicknessByWeight(false);
        if (apps[2])
            apps[2].setEdgeThicknessByWeight(false);
        if (apps[3])
            apps[3].setEdgeThicknessByWeight(false);
    }
});
...
function setEdgeDirection() {
    ...
    if (apps[0])
        apps[0].setEdgeDirection(value);
    if (apps[1])
        apps[1].setEdgeDirection(value);
    if (apps[2])
        apps[2].setEdgeDirection(value);
    if (apps[3])
        apps[3].setEdgeDirection(value);
}
...
resetDataSetIcon();
showVisIcons();
var apps = Array(null, null, null, null);
...
function setSurfaceOpacity() {
    ...
    if (apps[0])
        apps[0].setSurfaceOpacity(opacity);
    if (apps[1])
        apps[1].setSurfaceOpacity(opacity);
    if (apps[2])
        apps[2].setSurfaceOpacity(opacity);
    if (apps[3])
        apps[3].setSurfaceOpacity(opacity);
}
    ...
    if (loadObj.edgeSettings.colorBy === "none") {
        ...
        if (loadObj.edgeSettings.weight.type === "discrete") {
            ...
            for (var i = 0; i < setting.numCategory; i++) {
                ...
                document.getElementById('input-edge-discretized-' + i + '-color').color.fromString(setting.colorArray[i].substring(1));
            }
        }
        else {
...
function initNodeColor() {
    if ((loadObj.nodeSettings.nodeColorAttribute != null) && (loadObj.nodeSettings.nodeColorAttribute.length > 0)) {
        ...
        if (dataSet.attributes.info[loadObj.nodeSettings.nodeColorAttribute].isDiscrete) {
            ...
            document.getElementById('input-node-color').color.fromString(loadObj.nodeSettings.nodeColorDiscrete[0].substring(1));
            setNodeSizeOrColor();
        }
        else {
            document.getElementById('input-min-color').color.fromString(loadObj.nodeSettings.nodeColorContinuousMin.substring(1));
            document.getElementById('input-max-color').color.fromString(loadObj.nodeSettings.nodeColorContinuousMax.substring(1));
            setNodeSizeOrColor();
        }
    }
}
...
function loadBrainModel(file, callback) {
